"""Dump cn_xxl_canonical v5 best_model (79.46%) test logits, aligned to sigs."""
...
WORK = './work_dir/cn_xxl_canonical'
...
ep_saved = torch.load(CKPT, map_location='cpu').get('epoch', '?')
print(f'best_model from epoch {ep_saved}')
...
# Pull sigs from the standard test_depth_list.txt (same index order as bake).
...
test_sigs = [sig(l) for l in test_lines]
assert len(test_sigs) == len(ds), f'{len(test_sigs)} vs {len(ds)}'

model.pts_size = cfg['model_args']['pts_size']
all_log, all_lab = [], []
with torch.no_grad():
    for i, (pts, lab, _) in enumerate(loader):
        pts = pts.cuda().float()
        out = model(pts)
...
np.savez(OUT, logits=L, labels=Y, sigs=np.array(test_sigs))
