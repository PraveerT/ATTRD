"""
NvidiaPts10AuxQLoader — variant C loader.

Concatenates pts10 (10) + qcycprod broadcast (12) + target Q_combined_cycle
broadcast (4) into a single (T, 512, 26) tensor.

The MotionAuxQ model splits this:
  channels [0:22]  → main input (stage1_in_channels=22)
  channels [22:26] → target for aux quaternion-prediction loss
"""
...
class NvidiaPts10AuxQLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        prod_per_pt = np.broadcast_to(qprod[None, None, :], (T, N, 12)).copy()    # (T, N, 12)
        target = qprod[8:12]                                                       # Q_combined_cycle (4,)
        target_per_pt = np.broadcast_to(target[None, None, :], (T, N, 4)).copy()   # (T, N, 4)

        out = np.concatenate([pts, prod_per_pt, target_per_pt], axis=-1)  # (T, 512, 26)
