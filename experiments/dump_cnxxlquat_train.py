"""Dump cnxxlquat 91.08 logits on the 1050-sample NV train set (in EVAL mode,
...
OUT  = './work_dir/cn_xxl_quat_head/train_logits.npz'
...
# Build a TRAIN-set loader but EVAL-mode (use phase='valid' with valid_subject=None
# would still split; need phase='train' with no valid_subject to get the full
# 1050. NvidiaLoader docs: 'train' filters out valid_subject. So pass a
# nonexistent subject to keep all 1050.)
class TrainEvalLoader(DL):
    def __init__(self, **kw):
        super().__init__(**kw)
ds = TrainEvalLoader(phase='train', framerate=32, datatype='depth', valid_subject='_none_')
print('train samples:', len(ds))
assert len(ds) == 1050, f'expected 1050, got {len(ds)}'
...
print(f'shape={L.shape} train_acc={acc:.2f}% -> {OUT}')
