"""
NvidiaPts10QccSeqLoader — variant H1 loader.

Concatenates pts10 (10) + qcycprod broadcast (12) + per-frame q_combined
broadcast (4) into a (T, 512, 26) tensor. Last 4 channels VARY across t
(per-frame q_combined from <stub>_qcyc.npy), unlike auxq_loader where the
last 4 are a single global Q_combined_cycle.

Used by MotionQCCH1 to supply per-frame target quaternion sequence for
...
class NvidiaPts10QccSeqLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        qseq = np.load(qcyc_path).astype(np.float32)        # (T, 4) per-frame q_combined
        T, N, _ = pts.shape
        ...
        seq_per_pt = np.broadcast_to(qseq[:, None, :], (T, N, 4)).copy()  # varies per t

        out = np.concatenate([pts, prod_per_pt, seq_per_pt], axis=-1)  # (T, 512, 26)
