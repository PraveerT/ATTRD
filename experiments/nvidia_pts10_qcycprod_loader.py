"""
NvidiaPts10QcycprodLoader — pts10 (raw + nearest-lattice) plus per-sample
12-dim global cycle quaternion product
  [Q_raw_cycle (4), Q_lat_cycle (4), Q_combined_cycle (4)]
broadcast as 12 extra channels per point.

Input shape per sample: (T=32, 512, 22)
  channels [0:4]   = raw uvd-t
  channels [4:10]  = nearest-lattice (x, y, z, vx, vy, vz)
  channels [10:14] = Q_raw_cycle  (broadcast)
  channels [14:18] = Q_lat_cycle  (broadcast)
  channels [18:22] = Q_combined_cycle (broadcast)
"""
...
class NvidiaPts10QcycprodLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        out = np.concatenate([pts, prod_per_pt], axis=-1)  # (T, 512, 22)
        out_t = self.transform(out.reshape(-1, 22))
        if isinstance(out_t, torch.Tensor):
            out_t = out_t.view(T, N, 22)
        else:
            out_t = torch.from_numpy(out_t).view(T, N, 22)
