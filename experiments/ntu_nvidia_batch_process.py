...
def process_single_ntu_directory(args):
    """Process a single NTU directory using EXACT NVIDIA pipeline"""
...
def main():
    ...
    print(f"🚀 NTU Point Cloud Processing - EXACT NVIDIA Method")
    ...
    with mp.Pool(processes=num_processes) as pool:
        results = list(tqdm(
            ...
            total=len(all_dirs),
            desc="Processing NTU directories (NVIDIA method)"
        ))
    ...
    # Save processing log
    log_file = os.path.join(ntu_path, "nvidia_processing_log.txt")
    with open(log_file, 'w') as f:
        f.write("NTU Point Cloud Processing Log - EXACT NVIDIA Method\n")
    ...
    print("🎉 NVIDIA processing complete!")
    ...
    if successful > 0:
        ...
        print(f"  Ready for PMamba training!")
